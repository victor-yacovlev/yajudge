...
import 'package:yajudge_client/utils/csv_parser.dart';
...
class UsersImportCSVScreenState extends BaseScreenState {
  ...
  void _createUsersFromCsvData() {
    ...
    for (int i=0; i<_csvPreview!.length; i++) {
      ...
      User user = User()..defaultRole = UserRole_Student;
      user.firstName = row[firstNameCol];
      user.lastName = row[lastNameCol];
      if (midNameCol != -1)
        user.midName = row[midNameCol];
      if (emailCol != -1)
        user.email = row[emailCol];
      if (groupNameCol != -1)
        user.groupName = row[groupNameCol];
      else if (_groupForAll.text.trim().isNotEmpty)
        user.groupName = _groupForAll.text.trim();
      users.add(user);
    }
    _users = users;
  }
