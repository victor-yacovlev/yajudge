...
class CodeReviewManagementService extends CodeReviewManagementServiceBase {
  final log = Logger('SubmissionManager');
  ...
  Future<CodeReview> applyCodeReview(ServiceCall call, CodeReview request) async {
    ...
    CodeReview? existingReview = await _getCodeReviewForSubmission(request.submissionId.toInt());
    bool updateReview = false;
    if (existingReview!=null) {
      updateReview = true;
      bool sameReview = existingReview.contentEqualsTo(review);
      if (sameReview) {
        await parent.submissionManagementService.updateSubmissionStatus(
          call,
  ...
  Future<ReviewHistory> getReviewHistory(ServiceCall call, Submission request) async {
    ...
    for (final id in ids) {
      CodeReview? review = await _getCodeReviewForSubmission(id);
      if (review != null) {
        reviews.add(review);
      }
    }
